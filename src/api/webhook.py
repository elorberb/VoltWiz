"""
Webhook API for the VoltWiz application.
"""
...
def webhook():
    """
    Webhook endpoint for handling WhatsApp messages.
    """
    ...
    if incoming_msg.lower() in ['hi', 'hello', 'start', 'help']:
        ...
        resp.message("Welcome to the Electricity Provider Recommendation Bot! Let's find the best plan for you.")
        resp.message(conversation_handler.get_next_question(sender))
        return str(resp)
        ...
        if next_question:
            ...
            if user_data:
                ...
                if provider:
                    ...
                    resp.message(recommendation)
                else:
                    resp.message("Sorry, we couldn't find any suitable providers based on your requirements.")
                
                resp.message("\nTo start over, just send 'hi' or 'start'.")
            else:
                resp.message("Something went wrong. Please start over by sending 'hi'.")
    else:
        resp.message("To start over, just send 'hi' or 'start'.")
